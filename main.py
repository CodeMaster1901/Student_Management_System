...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:abhay2003@localhost/student_management_system'
db=SQLAlchemy(app)
...
def studentdetails():
    ...
    try:
        # Calling the stored procedure to get department-wise student counts
        department_wise_counts_query = text("CALL GetDepartmentWiseStudentCount()")
        department_wise_counts_result = db.session.execute(department_wise_counts_query)
        department_wise_counts = department_wise_counts_result.fetchall()

        # Closing the result set
        department_wise_counts_result.close()

    except Exception as e:
        # Handle exceptions
        print(f"Error: {e}")
        department_wise_counts = None

    # Assuming 'GetTotalStudentCount' is the name of your MySQL function
    total_student_count_query = text("SELECT GetTotalStudentCount() AS total_student_count")
    total_student_count_result = db.session.execute(total_student_count_query).fetchone()
    total_student_count = total_student_count_result.total_student_count
    return render_template('studentdetails.html',query=query,department_wise_counts=department_wise_counts,total_student_count=total_student_count)
